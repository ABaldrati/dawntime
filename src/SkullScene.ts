...
export class SkullScene extends AbstractScene {
    private static instance: Promise<SkullScene>;
    ...
    static async getInstance(): Promise<SkullScene> {
        if (!SkullScene.instance) {
            let scene = new SkullScene();
            SkullScene.instance = scene.buildScene();
        } else {
            let skullScene = await SkullScene.instance
            skullScene.buildGUI();
            skullScene.resetScene();
        }
    ...
    protected async buildScene(): Promise<SkullScene> {
        ...
        let skullPromise = loader.loadAsync(skullFile)
        skullPromise.catch(console.log)

        let skull: GLTF = await skullPromise
